                    ...
                    with st.spinner("Generating audio..."):
                        try:
                            ...
                            if st.session_state.current_practice_type == "Dialogue Practice":
                                conversation_parts.append(("announcer", st.session_state.current_question['Introduction'], "announcer"))

                                # Separate the conversation into multiple lines
                                conversation_text = st.session_state.current_question['Conversation']
                                lines = conversation_text.split("\n")

                                for line in lines:
                                    parts = line.split("：") # Split each line into speaker and text
                                    if len(parts) == 2:
                                        speaker = parts[0].strip()
                                        text = parts[1].strip()

                                        gender = "female" if "女性" in speaker else "male" if "男性" in speaker else "announcer"

                                        conversation_parts.append((speaker, text, gender))

                                conversation_parts.append(("announcer", st.session_state.current_question['Question'], "announcer"))
                            else:
                                conversation_parts.append(("announcer", st.session_state.current_question['Situation'], "announcer"))
                                conversation_parts.append(("announcer", st.session_state.current_question['Question'], "announcer"))
